...
public class BoundaryOfBinaryTree {
    ...
    public List<Integer> boundaryOfBinaryTree(TreeNode root) {
        ...
        res.add(root.val);
        for (int i = 0; i < left.size(); i++) res.add(left.get(i).val);
        for (int i = 1; i < leaves.size(); i++) res.add(leaves.get(i).val);
        for (int i = right.size() - 2; i > -1; i--) res.add(right.get(i).val);
        return res;
    }
    ...
    private void traverseRight(List<TreeNode> list, TreeNode node) {
        if (node == null) return;
        list.add(node);
        if (node.right != null) traverseRight(list, node.right);
        else traverseRight(list, node.left);
    }

    private void traverseLeft(List<TreeNode> list, TreeNode node) {
        if (node == null) return;
        list.add(node);
        if (node.left != null) traverseLeft(list, node.left);
        else traverseLeft(list, node.right);
    }
}
